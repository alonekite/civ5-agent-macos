...
| M6 deterministic turn executor | ADR-0015/0016/0017/0018/0022/0023 and TurnPlan schema 1 | Strict admission/report states, factual requirements, four-action ordered completion, pause/drift/rejection/error/unknown outcome, neutral event order, event-sink failure independence, watcher-protocol adaptation, read-only session-scoped completed-command lookup, and conservative cached-result/fresh-state reconciliation | No | Ordered core, bounded events, watcher adapter, and explicit no-retry recovery implemented offline; CLI loading pending; independent of knowledge/M5 |
