...
| M6 deterministic turn executor | ADR-0015/0016/0017/0018/0022 and TurnPlan schema 1 | Strict admission/report states, factual requirements, four-action ordered completion, pause/drift/rejection/error/unknown outcome, neutral event order, event-sink failure independence, watcher-protocol adaptation, and read-only session-scoped completed-command lookup | No | Ordered core, bounded events, watcher adapter, and recovery transport primitive implemented offline; CLI loading and explicit report recovery pending; independent of knowledge/M5 |
