...
| Session/match identity and turn journal | Bridge-session envelope, schema 1 store, and FireTuner-only opt-in watcher capture implemented under ADR-0017/0019/0020 | UUID/session rejection, private store, hash chain, tampering/truncation/size/concurrency/reopen, validated snapshot/result capture, database-source rejection, explicit resume, and journal-failure/no-retry tests | No | Initial capture implemented offline; remaining lifecycle/replay/export pending; no permanent save ID is claimed |
