...
| Session/match identity and turn journal | Bridge-session envelope, schema 1 store, FireTuner-only factual capture, payload-free verification, and append-order replay implemented under ADR-0017/0019/0020 | UUID/session rejection, private store, hash chain, tampering/truncation/size/concurrency/reopen, validated capture, database-source rejection, explicit resume, verification privacy, detached ordered replay, explicit payload acknowledgement, and journal-failure/no-retry tests | No | Capture, verification, and replay implemented offline; file export pending; no permanent save ID is claimed |
