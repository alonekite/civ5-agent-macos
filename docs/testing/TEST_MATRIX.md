...
| Session/match identity and turn journal | Bridge-session envelope, schema 1 store, FireTuner-only factual capture, payload-free verification, append-order replay, and ADR-0021 structural export implemented | UUID/session rejection, private store, hash chain, tampering/truncation/size/concurrency/reopen, validated capture, database-source rejection, explicit resume, verification privacy, detached replay, payload gate, repeatable private export, overwrite/symlink/partial-write rejection, and journal-failure/no-retry tests | No | M5 implemented offline; bounded live capture/verify/export pending; no permanent save ID or anonymity is claimed |
...
| M6 deterministic turn executor | ADR-0015/0016/0017/0018/0022/0023/0024 and TurnPlan schema 1 | Strict admission/report states, factual requirements, four-action ordered completion, pause/drift/rejection/error/unknown outcome, neutral event order, event-sink failure independence, watcher-protocol adaptation, read-only session-scoped completed-command lookup, conservative cached-result/fresh-state reconciliation, and bounded exact-field CLI loading | No | All acceptance criteria implemented offline, including watcher-only validation/execution CLI and explicit no-retry recovery; bounded live verification pending; independent of knowledge/M5 |
| M7 public API stabilization | Cross-module inventory reviewed against current contracts and exports | Documentation-link validation | N/A | Inventory complete; bridge client, supported import surface, exception taxonomy, and compatibility tests pending |
