...
| Session/match identity and turn journal | Bridge-session envelope implemented; match journal proposed under ADR-0017/0019 | Canonical UUIDv4 generation/validation, stable handler identity, read exposure, missing/stale write rejection, audit propagation | No | Bridge session implemented offline; match binding/journal pending; no permanent save ID is claimed |
