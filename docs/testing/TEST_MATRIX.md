...
| Session/match identity and turn journal | Bridge-session envelope, schema 1 store, FireTuner-only factual capture, and payload-free full-chain verification implemented under ADR-0017/0019/0020 | UUID/session rejection, private store, hash chain, tampering/truncation/size/concurrency/reopen, validated snapshot/submission/result/failure/transition capture, database-source rejection, explicit resume, verification privacy, and journal-failure/no-retry tests | No | Capture and verification implemented offline; replay/export pending; no permanent save ID is claimed |
