...
| Session/match identity and turn journal | Bridge-session envelope plus journal schema 1 codec/store implemented under ADR-0017/0019/0020 | UUID validation, stable handler identity, missing/stale/unbound rejection, private permissions, canonical round trip, hash chain, tampering/truncation/size/concurrency/reopen tests | No | Core implemented offline; watcher/CLI capture pending; no permanent save ID is claimed |
