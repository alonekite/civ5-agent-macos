...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | schema 7 只读模型及 WB-S01–S06 已离线实现；两个读取程序均低于 1,000 字节且不改变选择；写命令、结果验证和 C5 仍未实现 | 仅在 C1–C5 全部通过后执行清单第 8 节：确认只读不改变选择，完成一次错误源坐标的写前拒绝，再经单独确认执行一次候选建造；任一精确成功分支、独立 watcher、私有审计和完整恢复均须通过 |
