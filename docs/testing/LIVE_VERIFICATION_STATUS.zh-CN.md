...
| 能力 | 离线状态 | 下一次真实游戏测试 |
|---|---|---|
| M5 对局事实日志 | 私有追加存储、哈希链、验证、顺序回放和脱敏结构导出均已通过离线测试 | 按 M8 联合清单捕获一个快照、命令生命周期和回合转换；恢复主机后验证、回放并导出 |
| M6 确定性回合执行器 | 显式 TurnPlan 的校验、执行、暂停、陈旧状态和未知结果恢复路径均已通过离线测试 | 对同一 watcher 创建只含 `end_turn` 的显式计划；先校验，且只执行一次，要求完整报告与回合推进 |
