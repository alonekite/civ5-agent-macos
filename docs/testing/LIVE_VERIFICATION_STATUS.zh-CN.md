...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | C1–C5 离线门槛已完成。两次 C6 均在只读阶段安全停止：首次证明 `GameInfoActions` 是索引表；第二次证明目标 `unit:CanBuild` 绑定的可选标志必须是数字。两次均未发送命令或游戏写入，主机完整恢复 | ADR-0034 将读取和写前守卫统一为 `CanBuild(..., 0, 1)`；需重跑 C1–C5、推送并通过 CI，然后由操作者重新授权第 8 节。新一轮仍只允许只读证明、一次错误源坐标拒绝和一次单独授权建造 |
