...
| M5 对局事实日志 | 三次联合测试已实机确认 schema 5 捕获、哈希链、连续回放、结构导出、私有权限、人工回合转换，以及多次失败命令的提交/结果/验证错误完整生命周期 | 仍需在同一次成功自动推进中确认成功命令生命周期和回合转换 |
| M6 确定性回合执行器 | 显式计划创建/校验、有界标记、陈旧计划写前拒绝及 ADR-0029 命名枚举均已实机验证；第三次实际执行后因新出现的工人要求而精确失败并保留变化后状态 | 需在无自动/延迟单位任务的早期新局复测自动推进 |
...
| M6 确定性回合执行器 | 显式 TurnPlan 的校验、执行、暂停、陈旧状态、程序上限、命名无阻塞枚举和未知结果恢复均已测试；实机失败路径已覆盖 | 在无自动任务的最小早期新局新建计划并单次执行，要求完整报告与自动回合推进 |
