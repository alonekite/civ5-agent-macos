...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | C1–C5 离线门槛已完成。2026-09-18 首次 C6 在只读阶段安全停止：目标运行时的 `GameInfoActions` 是表，不能作为函数迭代；未发送任何命令，主机已完整恢复 | 先将候选读取修正为数值索引表遍历，重跑 C1–C5 并推送；然后需操作者重新授权第 8 节。新一轮仍只允许只读证明、一次错误源坐标拒绝和一次单独授权建造 |
