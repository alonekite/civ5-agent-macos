...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | C1–C5 离线门槛已完成：只读模型、单次受控提交、双分支事实验证、确定性 TurnPlan、335 项测试、重复产物检查/隔离安装和隐私扫描均通过；读取程序及 996 字节最坏写程序均在上限内 | 现可在操作者另行授权后执行清单第 8 节：确认只读不改变选择，完成一次错误源坐标的写前拒绝，再经单独确认执行一次候选建造；任一精确成功分支、独立 watcher、私有审计和完整恢复均须通过 |
