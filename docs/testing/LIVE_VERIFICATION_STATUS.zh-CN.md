...
| 能力 | 离线状态 | 下一次真实游戏测试 |
|---|---|---|
| 相邻普通坐标移动 | schema 6 受限候选目标已离线实现；写命令尚未实现。ADR-0032 选定 `UI.SelectUnit` + `Game.SelectionListMove` | 写命令与完整离线门槛通过后，按清单第 7 节先验证源坐标拒绝，再经用户明确授权执行一次相邻空地移动；任何不确定结果均停止且不重试 |

M5/M6 联合实机发布门槛已经完成；清单第 6 节保留为回归程序。
