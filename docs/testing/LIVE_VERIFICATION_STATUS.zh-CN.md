...
| 外部框架 SessionSpec v2 UI 启动序列 | 多次失败均在投递前安全终止；持久加固与恢复保持正确。一次无人值守尝试暴露了复用旧确认的授权链错误，但没有发生 UI 投递 | 必须通过 ADR-0054 的 checkpoint 创建后、同任务、明确在场、step+nonce 一次性门禁，重新完成 `PLAY` 与 `Click to Continue`；旧确认不得复用 |
