...
| schema 8：普通科研运行时事实 | 首次 C4 已确认 schema 8 分段读取和无可见读取副作用，并通过只读诊断确认精确 times-100 进度属于 `CvTeamTechs`；主机基线完整恢复 | 在 ADR-0037 修正后的代码上重新验证 supported 状态、UI 数值、溢出和零命令审计；首次尝试不是成功门禁 |
