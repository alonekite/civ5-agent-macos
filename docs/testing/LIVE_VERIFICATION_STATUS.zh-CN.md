...
| M5 对局事实日志 | 两次联合测试已实机确认 schema 5 捕获、哈希链、连续回放、结构导出、私有权限、人工回合转换，以及失败命令的提交/结果/验证错误完整生命周期 | 仍需在同一次成功自动推进中确认成功命令生命周期和回合转换 |
| M6 确定性回合执行器 | 显式单动作 TurnPlan 已在真实 watcher 会话中创建/校验；有界 Lua 已完整到达并返回标记；陈旧计划复用在写前被拒绝 | 第二次发现把无阻塞错误假定为数值 0；ADR-0029 已改用游戏枚举，仍需复测自动推进 |

## 待实机验证

| 能力 | 离线状态 | 下一次真实游戏测试 |
|---|---|---|
| M5 对局事实日志 | 私有追加存储、哈希链、验证、顺序回放、脱敏结构导出及未知结果记录均已通过离线测试 | 新会话中要求成功结果与自动回合转换形成同一完整生命周期 |
| M6 确定性回合执行器 | 显式 TurnPlan 的校验、执行、暂停、陈旧状态、程序上限、命名无阻塞枚举和未知结果恢复均已离线测试 | 用 ADR-0029 修正后的 `end_turn` 新建计划并单次执行，要求完整报告与自动回合推进 |
