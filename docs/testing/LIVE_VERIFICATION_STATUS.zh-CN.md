...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | 两轮最新 C6 均确认候选/UI 一致、读取无副作用和错误源坐标写前拒绝。词法修复后的唯一写入成功解析，但因按 build type 字符串索引数字表 `GameInfoActions` 而返回明确 `invalid_build`；前后状态与 UI 均无变化，无重试并完整恢复 | 改为数字索引遍历并精确匹配 Type/SubType/MissionData，重跑 C1–C5、推送并通过 CI，然后由操作者在全新会话重新授权第 8 节；不得复用或重试失败命令 |
