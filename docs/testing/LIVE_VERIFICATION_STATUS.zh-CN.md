...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | 最新 C6 已确认 schema 7 候选与游戏 UI 一致、读取无副作用，错误源坐标在写入前拒绝且状态不变。随后单独授权的唯一一次写入因生成 Lua 把数字与 `or`/`then` 相连而在解析阶段失败；无有效 marker，UI 未开始建造、移动力未下降，并严格执行不重试和完整恢复 | 修复数字/关键字词法边界，重跑 C1–C5、推送并通过 CI，然后由操作者在全新会话重新授权第 8 节；不得复用或重试失败命令 |
