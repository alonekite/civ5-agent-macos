...
| 能力 | 当前证据 | 后续实机门槛 |
|---|---|---|
| schema 7 与 `worker_build` | schema 7 只读模型、C2 单次受控提交和 C3 双分支事实验证已离线实现；读取程序及 996 字节最坏写程序均在上限内；C4/C5 仍未完成 | 仅在 C1–C5 全部通过后执行清单第 8 节：确认只读不改变选择，完成一次错误源坐标的写前拒绝，再经单独确认执行一次候选建造；任一精确成功分支、独立 watcher、私有审计和完整恢复均须通过 |
