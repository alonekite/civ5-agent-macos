...
| R-018 | Selection-dependent worker legality or immediate completion makes the wrong build, wrong unit, or ambiguous result appear successful | Medium | High | Mitigated by design; blocks advertising until live evidence | ADR-0033 and D2 freeze selection-free unit candidates, exact selected-unit stock dispatch, blank featureless land, ordinary non-consuming builds, lower-movement proof, separate active/completed postconditions, bounded Lua, and UUID no-retry behavior; D3 freezes WB-S01–A02 plus a single-write operator gate; two C6 reads safely exposed indexed action-table and integer `CanBuild` flag requirements, both are repaired offline under ADR-0034, and fresh C6 proof remains required |
