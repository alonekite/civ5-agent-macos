...
| R-018 | Selection-dependent worker legality or immediate completion makes the wrong build, wrong unit, or ambiguous result appear successful | Medium | High | Open; blocks M10 command code | D1 must prove conservative per-unit candidates without read-side selection, exact selected-unit stock dispatch within 1,000 bytes, and separate active-build/immediate-completion postconditions; exclude special build classes and preserve UUID no-retry behavior until bounded target evidence passes |
