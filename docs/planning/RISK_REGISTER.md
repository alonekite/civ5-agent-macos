...
| R-020 | A stale session initiation or manual completion claim is treated as authorization or game-state proof | Medium | High | Open; offline mitigation only | ADR-0061 scopes PLAY to one short-lived task/session/spec/checkpoint grant and manual completion to a separate post-request one-use challenge; composition root must enforce message provenance, protected live preflight and exact process identity before gate pass, then require a same-session active-turn read-only probe. No target evidence yet |
